	...
	return (
		<div className='relative cursor-pointer' onMouseEnter={handleMouseEnter} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
			...
			text-lg w-8 h-8 absolute text-white bg-green-600 rounded-bl-lg hover:scale-125 z-10 transition ease-out right-0'>
					<a href={anime.url.toString()} target='_blank' className='h-6 w-6'>
						<GlobeAltIcon/>
					</a>
				</div>}
				<img className='w-48 h-60' src={anime.images?.jpg?.image_url.toString()} />
				<div className='text-slate-200 text-lg p-2 bg-slate-800'>
					<div className='font-semibold overflow-hidden whitespace-nowrap overflow-ellipsis w-36'>{anime.title.toString()}</div>
					<div className='flex justify-between mx-1'>
						<span>{anime.type?.toString()}</span>
						<span>{anime.rating && ratingToStub(anime.rating.toString())}</span>
					</div>
				</div>
			</div>
		</div>
